...
import { 
  ...
  Phone, BookOpen, GraduationCap, ArrowRight, UserCircle2, ShieldCheck
} from 'lucide-react';
      ...
      <div className="lg:w-1/2 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="text" placeholder={isAdmin ? "Username" : "Student Number"} className="w-full px-8 py-5 rounded-[2rem] bg-slate-50 border border-slate-100 font-black" onChange={(e) => setLoginId(e.target.value)} />
...
function Dashboard({ role, student }: { role: 'admin' | 'student', student: StudentRecord | null }) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // FIX: Cast options to 'any' to avoid the TypeScript build error on Vercel
      const sheets = await readXlsxFile(file, { getSheets: true } as any);
      ...
      if (masterData.length === 0) throw new Error("No valid data found.");

      const { error: delError } = await supabase.from('student_ledger').delete().neq('id', 'temp-init');
      if (delError) throw delError;

      const { error: insError } = await supabase.from('student_ledger').insert(masterData);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <nav className="h-20 bg-white border-b flex items-center px-8 justify-between sticky top-0 z-50">
        ...
        <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="text-red-500 font-black text-[10px] uppercase tracking-widest bg-red-50 px-4 py-2 rounded-xl"> <LogOut size={14}/> Log Out</button>
      </nav>
          ...
          <div className="bg-white rounded-[3.5rem] p-12 shadow-xl border border-blue-100">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-slate-400 font-bold italic text-sm mt-1 uppercase">Duplicate Handling: Active</p>
                </div>
            </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-6 duration-700">
             <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
                <div className="flex justify-between items-start mb-4">
                    <p className="text-blue-400 font-black text-[10px] uppercase tracking-[0.3em]">Fees Outstanding</p>
                ...
                <h2 className="text-8xl font-black italic mb-8 leading-none tracking-tighter">${student?.total_outstanding.toLocaleString()}</h2>
             ...
             <div className="grid lg:grid-cols-3 gap-6">
                <BalanceBox label="Previous Balance" val={student?.previous_balance || 0} />
                <BalanceBox label="Term 3 2025" val={student?.term_3_balance || 0} />
                <BalanceBox label="Term 1 2026" val={student?.term_1_2026_balance || 0} />
             </div>
          </div>
        )}
      </main>
...
function BalanceBox({ label, val }: { label: string, val: number }) {
    return (
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest italic mb-2">{label}</p>
            <p className="text-3xl font-black italic text-slate-900">${val.toLocaleString()}</p>
        </div>
    );
}
