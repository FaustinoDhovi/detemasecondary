...
import { Loader2, ArrowLeft, ShieldAlert, LogIn, UserPlus, CheckCircle, GraduationCap, Briefcase } from 'lucide-react';
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (view === 'staff-login') {
        ...
        // ROLE-BASED REDIRECTION INCLUDING BURSAR
        if (profile.role === 'admin') {
            router.push('/portal/admin');
        } else if (profile.role === 'bursar') {
            router.push('/portal/bursar'); // Redirects to the Bursar dashboard
        } else {
            router.push('/teacher');
        }
      } else {
    ...
    try {
      ...
      if (authUser.user) {
        await supabase.from('staff_profiles').insert([{
          ...
          // If the master list says 'BURSAR', use that role, otherwise default to 'teacher'
          role: validStaff.designation?.toLowerCase().includes('bursar') ? 'bursar' : 'teacher',
          is_approved: false
        }]);
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          {view !== 'staff-signup' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button disabled={loading} className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-600 transition-all flex justify-center items-center gap-3 shadow-xl">
            ...
            <form onSubmit={handleStaffSignUp} className="space-y-4">
              ...
              <button disabled={loading} className="w-full bg-emerald-600 text-white py-6 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-900 transition-all flex justify-center items-center gap-3 shadow-lg">
